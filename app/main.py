...
def read_report(path: Path) -> tuple[SimpleNamespace, list[dict]] | None:
    try:
...
def bundled_reports() -> list[tuple[SimpleNamespace, list[dict]]]:
    reports = []
...
def startup() -> None:
    # The public site is file-first: a temporary database outage must not block
    # Railway's health check or prevent newly committed JSON from appearing.
    try:
        init_db()
...
def history(request: Request):
    ...
    return templates.TemplateResponse(
        "history.html", {"request": request, "reports": reports}
    )
